...
import { Search, Bell, MessageSquare, RefreshCw, User, LogOut, Settings } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-20 shadow-2xs">
      {/* Search Input */}
      ...
      <div className="flex items-center gap-3">
        {/* System Clock */}
        <span className="hidden sm:inline-block text-xs font-mono font-medium text-slate-600 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-md">
          {clock || '00:00:00'}
        </span>

        {/* Role Selector */}
        <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-md px-2 py-1">
          <span className="text-[10px] text-slate-400 font-semibold uppercase px-1">View:</span>
          <select
            value={currentRole}
        ...
        <button
          onClick={onReset}
          className="flex items-center gap-1.5 text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 px-2.5 py-1.5 rounded-md transition-colors cursor-pointer shadow-2xs"
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="w-8 h-8 rounded-md bg-blue-600 text-white flex items-center justify-center font-bold text-xs shadow-2xs">
